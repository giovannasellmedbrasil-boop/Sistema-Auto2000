...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await getLeadById(id))) return NextResponse.json({ error: "Lead não encontrado" }, { status: 404 });
  ...
  switch (data.action) {
    ...
    case "testDrive": {
      ...
      const lead = await advanceLeadStage(id, stageMap[data.action]);
      return NextResponse.json({ lead });
    }
    case "proposal": {
      const lead = await advanceLeadStage(id, "proposal", { proposalValue: data.proposalValue });
      return NextResponse.json({ lead });
    }
    case "sold": {
      const result = await markLeadSold(id, { finalPrice: data.finalPrice, paymentMethod: data.paymentMethod });
      if (!result) {
    ...
    case "lost": {
      const lead = await markLeadLost(id, data.reason);
      return NextResponse.json({ lead });
    }
    case "assign": {
      const lead = await updateLead(id, { ownerId: data.ownerId });
      return NextResponse.json({ lead });
    }
  }
}
